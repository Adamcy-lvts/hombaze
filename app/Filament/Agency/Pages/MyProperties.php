...
class MyProperties extends Page implements HasActions, HasForms
{
    ...
    protected string $view = 'filament.agency.pages.my-properties';
    
    // Hide from sidebar navigation (we only want it on mobile bottom nav)
    protected static bool $shouldRegisterNavigation = false;

    protected static ?string $slug = 'my-properties';
    ...
    public function loadProperties()
    {
        ...
        if ($agency) {
            $this->properties = Property::where('agency_id', $agency->id)
                ->orderBy('created_at', 'desc')
                ->with(['media', 'propertyType', 'agent.user'])
                ->get();
        } else {
