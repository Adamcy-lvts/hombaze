...
class CreateProperty extends Page
{
    ...
    protected string $view = 'filament.agent.pages.create-property';
    ...
    public function getOwnersProperty() {
        ...
        return PropertyOwner::where('agent_id', $agent->id)->orderBy('first_name')->orderBy('company_name')->get();
    }
    ...
    public function create() {
        Log::info('Agent CreateProperty: Starting creation for user ' . Auth::id());
        ...
        if (!$agentProfile) { Notification::make()->danger()->title('Error')->body('Agent profile not found.')->send(); return; }
        ...
        try {
            $property = DB::transaction(function () use ($data, $agentProfile) {
                $ownerId = $this->owner_id;
                if ($this->createNewOwner) {
                    $ownerData = [
                        'type' => $this->owner_type, 'email' => $this->owner_email, 'phone' => $this->owner_phone,
                        'agent_id' => $agentProfile->id, 'is_active' => true,
                    ];
                    if ($this->owner_type === 'individual') {
                ...
                $property = Property::create(array_merge($data, [
                    ...
                    'agent_id' => $agentProfile->id, 'owner_id' => $ownerId, 'is_published' => true,
                ]));
            ...
            Log::error('Agent CreateProperty: Transaction failed', ['error' => $e->getMessage()]);
    ...
    public function backToProperties() { $this->resetFormState(); return redirect()->route('filament.agent.pages.my-properties'); }

    private function resetFormState() {
        $this->reset(['step', 'selectedCategory', 'property_type_id', 'property_subtype_id', 'propertyTitle', 'propertySlug', 'price', 'listing_type', 'status', 'owner_id', 'createNewOwner', 'owner_type', 'owner_first_name', 'owner_last_name', 'owner_company_name', 'owner_email', 'owner_phone', 'bedrooms', 'description', 'state_id', 'city_id', 'area_id', 'areaSearch', 'address', 'featured_image', 'gallery_images', 'gallery_captions', 'createdPropertyId', 'createdPropertySlug', 'plot_size_id', 'custom_plot_size', 'custom_plot_unit', 'useCustomPlotSize']);
    }
}
