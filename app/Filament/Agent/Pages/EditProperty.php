...
class EditProperty extends Page
{
    ...
    protected string $view = 'filament.agent.pages.edit-property';
    ...
    {
        if (!$this->record) {
             $this->redirect(route('filament.agent.pages.my-properties')); return;
        }
        ...
        $user = Auth::user();
        if (!$user->agentProfile || $this->property->agent_id !== $user->agentProfile->id) {
    ...
    public function updateProperty()
    {
        ...
        return redirect()->route('filament.agent.pages.my-properties');
    }

    public function cancel() { return redirect()->route('filament.agent.pages.my-properties'); }
}
