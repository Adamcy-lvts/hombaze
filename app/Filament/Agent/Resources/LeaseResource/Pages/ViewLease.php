...
class ViewLease extends ViewRecord
{
    ...
    protected string $view = 'filament.agent.pages.lease-view';
        ...
        try {
            ...
            if ($data['template_id']) {
                $template = LeaseTemplate::where('id', $data['template_id'])
                    // Check ownership/access. For Landlord panel it was 'landlord_id' => Auth::id()
                    // For Agent, we might need to be more permissive or check agent relation
                    // Keeping it consistent with the blade view logic (Auth::id())
            ...
            if ($template) {
                ...
                $leaseContent = $template->substituteVariables([
                    ...
                    'landlord_name' => $lease->landlord->name, // Assuming relation exists
