...
use App\Filament\PropertyOwner\Resources\PropertyResource;
...
class CreateProperty extends Page
{
    ...
    protected string $view = 'filament.property-owner.pages.create-property';
    ...
    public function create()
    {
        ...
        try {
            $property = DB::transaction(function () use ($data, $propertyOwner) {
                // Determine moderation status based on owner verification
                $moderationStatus = $propertyOwner->is_verified ? 'approved' : 'pending';
        ...
        if ($property->moderation_status === 'pending') {
            Notification::make()
                ...
                ->body('Your property has been submitted and is pending approval. It will be visible once reviewed by our team.')
                ->send();
        } else {
            Notification::make()
                ->success()
                ->title('Property Created Successfully')
                ->body('Your property is now live and visible to potential buyers/renters.')
                ->send();
        }
