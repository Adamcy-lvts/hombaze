        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            ...
            <div>
                <span class="relative z-0 inline-flex shadow-xl rounded-2xl bg-white/80 backdrop-blur-xl border border-white/30 overflow-hidden">
                    ...
                    @else
                        <a href="{{ $paginator->previousPageUrl() }}" rel="prev" class="relative inline-flex items-center px-4 py-3 text-sm font-semibold text-gray-600 bg-white/80 hover:bg-gradient-to-r hover:from-emerald-500 hover:to-teal-500 hover:text-white focus:z-10 focus:outline-none focus:ring-4 focus:ring-emerald-500/20 transition-all duration-300 group" aria-label="Previous">
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="relative inline-flex items-center px-4 py-3 text-sm font-bold text-white bg-gradient-to-r from-emerald-600 to-teal-600 cursor-default shadow-lg">{{ $page }}</span>
                                    </span>
                                @else
                                    <a href="{{ $url }}" class="relative inline-flex items-center px-4 py-3 text-sm font-semibold text-gray-600 bg-white/80 hover:bg-gradient-to-r hover:from-emerald-50 hover:to-teal-50 hover:text-emerald-600 focus:z-10 focus:outline-none focus:ring-4 focus:ring-emerald-500/20 transition-all duration-300 transform hover:scale-105" aria-label="Go to page {{ $page }}">
                                        {{ $page }}
                                    </a>
                                @endif
                            @endforeach
