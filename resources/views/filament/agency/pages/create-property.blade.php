                ...
                <div class="flex-1 relative overflow-hidden px-6">
                    ...
                    >
                        ...
                        <div class="space-y-5">
                                    ...
                                    <div class="space-y-3 animate-fade-in">
                                        ...
                                        <div class="grid grid-cols-2 gap-3">
                                            <div>
                                                ...
                                                @error('owner_email') <span class="text-danger-600 text-xs">{{ $message }}</span> @enderror
                                            </div>
                                            <div>
                                                ...
                                                @error('owner_phone') <span class="text-danger-600 text-xs">{{ $message }}</span> @enderror
                                            </div>
                                        </div>
                                    </div>
                                @endif
                            ...
                            {{-- Agent Assignment (Agency Panel Specific) --}}
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Assign to Agent</label>
                                <select wire:model="agent_id" class="w-full rounded-xl border-gray-300 bg-white text-gray-900 dark:border-gray-700 dark:bg-gray-900 dark:text-white focus:border-primary-500 focus:ring-1 focus:ring-primary-500/50 transition-colors py-3.5 px-4 pr-10 outline-none">
                                    <option value="">Select Agent</option>
                                    @foreach($this->agents as $agentId => $agentName)
                                        <option value="{{ $agentId }}">{{ $agentName }}</option>
                                    @endforeach
                                </select>
                                @error('agent_id') <span class="text-danger-600 text-xs block mt-1">{{ $message }}</span> @enderror
                            </div>
                        ...
                        <div class="relative group cursor-pointer flex-1 min-h-[300px] border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-3xl flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-800/50 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors overflow-hidden"
                             x-data="{
                                ...
                                async handleImageUpload(e) {
                                    ...
                                    try {
                                        if (typeof window.validateAndProcessImage !== 'function') throw new Error('Image processor not loaded.');
                                ...
                                <div class="grid grid-cols-2 gap-4 mt-4">
                                    @foreach($gallery_images as $index => $image)
                                        <div class="relative group bg-white dark:bg-gray-800 p-2 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
                                            ...
                                            <div>
                                                <input type="text" wire:model="gallery_captions.{{ $index }}" placeholder="Caption (e.g. Kitchen)"
                ...
                reader.onload = (e) => {
                    ...
                    img.onload = () => {
                        ...
                        if (width < MIN_WIDTH || height < MIN_HEIGHT) {
                            reject(`This photo is too small. Please select a photo at least ${MIN_WIDTH}x${MIN_HEIGHT}px.`);
                            return;
                        }
                        ...
                        canvas.toBlob((blob) => {
                            ...
                            const newFile = new File([blob], safeName, { type: 'image/jpeg', lastModified: Date.now() });
                            resolve(newFile);
                        }, 'image/jpeg', QUALITY);
                    };
